...
NTSTATUS GetRegistryDWORD(HANDLE hKey,
                          LPCWSTR Value,
                          PULONG Data,
                          ULONG Default);
NTSTATUS QueryAndAllocRegistryData(HANDLE hKey,
                                   LPCWSTR Value,
                                   ULONG Type,
...
NTSTATUS InitializeConfigs(PUNICODE_STRING RegistryPath) {
  ...
  QueryAndAllocRegistryData(hkey, L"Hid_HideFsDirs", REG_MULTI_SZ,
                            &config.hideFSDirs, NULL);
  QueryAndAllocRegistryData(hkey, L"Hid_HideFsFiles", REG_MULTI_SZ,
                            &config.hideFSFiles, NULL);
  QueryAndAllocRegistryData(hkey, L"Hid_HideRegKeys", REG_MULTI_SZ,
                            &config.hideRegKeys, NULL);
  QueryAndAllocRegistryData(hkey, L"Hid_HideRegValues", REG_MULTI_SZ,
                            &config.hideRegValues, NULL);

  QueryAndAllocRegistryData(hkey, L"Hid_IgnoredImages", REG_MULTI_SZ,
                            &config.ignoreImages, NULL);
  QueryAndAllocRegistryData(hkey, L"Hid_ProtectedImages", REG_MULTI_SZ,
                            &config.protectImages, NULL);
  QueryAndAllocRegistryData(hkey, L"Hid_HideImages", REG_MULTI_SZ,
                            &config.hideImages, NULL);

  ZwClose(hkey);

  g_configContext =
      ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(config), CONFIG_ALLOC_TAG);
  if (!g_configContext) {
...
NTSTATUS CfgEnumConfigsTable(enum CfgMultiStringTables Table,
                             CfgMultiStringCallback Callback,
                             PVOID Params) {
...
NTSTATUS GetRegistryDWORD(HANDLE hKey,
  ...
  status = ZwQueryValueKey(hKey, &valueName, KeyValuePartialInformation, buffer,
...
NTSTATUS QueryAndAllocRegistryData(HANDLE hKey,
  ...
  if (Default) {
    ...
    dataBuffer =
        ExAllocatePool2(POOL_FLAG_NON_PAGED, dataLength, CONFIG_ALLOC_TAG);
    if (!dataBuffer)
      return STATUS_NO_MEMORY;
  ...
  status = ZwQueryValueKey(hKey, &valueName, KeyValuePartialInformation, NULL,
                           0, &length);
  ...
  info = (PKEY_VALUE_PARTIAL_INFORMATION)ExAllocatePool2(
      POOL_FLAG_NON_PAGED, length, CONFIG_ALLOC_TAG);
  if (!info)
    goto end_proc;

  status = ZwQueryValueKey(hKey, &valueName, KeyValuePartialInformation, info,
                           length, &length);
  if (!NT_SUCCESS(status))
    goto end_proc;
  ...
  dataBuffer = ExAllocatePool2(POOL_FLAG_NON_PAGED, dataLength, CONFIG_ALLOC_TAG);
  if (!dataBuffer) {
